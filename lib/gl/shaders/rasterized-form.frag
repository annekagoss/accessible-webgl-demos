...
void main() {
  ...
  float noiseScale = mix(4., 1.1, noiseC);
  ...
  imageSt1 = clamp(imageSt1, vec2(0.2, 0.1), vec2(.8, .9));

	float noiseAmt = mix(10.0, 1.0, c);
    vec2 noiseCoordinate = applyNoise(imageSt, noise * noiseAmt);
	
  imageSt = mix(imageSt, noiseCoordinate, 0.05);
  
	vec2 noiseCoordinate1 = applyNoise(imageSt1, noise * noiseAmt * .5);
	
  imageSt1 = mix(imageSt1, noiseCoordinate1, 0.05);
  ...
  vec4 color;
  if (backgroundLuminance > .5) {
    color = mix(uColor, vec4(image.rgb, 1.), image.a);
  } else {
    color = uColor + image;
  }
